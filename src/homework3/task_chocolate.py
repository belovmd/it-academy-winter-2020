...
# определяет наименьшее число отломанных долек с помощью t разломов
breaks = []

if k < n * m:
    # количество делений на части
    if k // n and not k % n or k // m and not k % m:
        breaks = [k]

    # количество делений между дольками
    if k % m or k % n:
        # крайнее деление между дольками
        if not (k + 1) % m or not (k + 1) % n:
            breaks += [k + 1]
        else:
            # если делится последняя часть шоколадки от деления на части
